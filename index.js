function getWeatherByCity() {
    ...
    return fetch(`http://api.openweathermap.org/data/2.5/weather?q=${city}&APPID=${clientKey}`)
        ...
        .then(json => {
            if (json.cod === 200) {
                document.getElementById('weather').innerHTML = prepareTemplate('entry-template')(json);
            } else {
                document.getElementById('error').innerHTML = prepareTemplate('entry-template-err')(json);
            }
        })
        .catch(e => {
            var errorObject = {};
